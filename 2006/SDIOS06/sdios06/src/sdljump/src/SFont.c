...
/*******************************************************************************
 *      MODIFIED BY JUAN PEDRO BOLIVAR. May not work outside his projects      *
 ******************************************************************************/
...
SFont_Font* SFont_InitFont(char * file, SDL_Surface* Surface, Uint8 alpha)
{
    int x = 0, i = 0;
    Uint32 pixel;
    SFont_Font* Font;
	...
	Font->Surface = JPB_LoadImg(file, alpha, 0, 0);
...
int charWidth(SFont_Font* Font, char c)
{
...
int findWord(SFont_Font* Font, char *text, int i)
{
	char *c;
	for (c = text+i; *c != ' ' && *c != '/' && *c != '\\'
			&& *c != '\0'; c++) {
...
int wordWidth(SFont_Font* Font, char *text, int i, int end)
{
	int width = 0;
	for (;i<end; i++) {
...
void copyWord(SFont_Font* Font, char *text, char* dest, int start, int end)
{
	int i;
	int len = strlen (dest);
						...
						int gap, int align, char *text)
{
    ...
    for(i = 0; i < len; i++) {
		...
		if ((width + ww) >= w) {
			switch (align) {
			case ALEFT:
				SFont_Write(Font, x, y, buf); break;
			case ARIGHT:
				SFont_Write(Font, x+w-width, y, buf); break;
			case ACENTER:
				SFont_Write(Font, x+(w-width)/2, y,buf); break;
			default:
				break;
			}
			width = 0;
...
int SFont_FillWith(SFont_Font *Font, int x, int y, int w, char c)
{
	...
	for (i = x+(x%charw); i <= x+w-charw; i += charw) {
		srcrect.w = dstrect.w = 
		    ...
		    (Font->CharPos[charoffset] + Font->CharPos[charoffset-1])/2;
		srcrect.x = (Font->CharPos[charoffset]+Font->CharPos[charoffset-1])/2;
		dstrect.x = i - (float)(Font->CharPos[charoffset]
				      - Font->CharPos[charoffset-1])/2;
...
void SFont_WriteMaxWidth (SFont_Font * Font, int x, int y, int w, int align, char* tag, char* text)
{
